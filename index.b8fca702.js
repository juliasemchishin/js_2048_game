...
function renderBoard() {
    ...
    if (statusElement === "win") messageWin.classList.remove("hidden");
    else if (statusElement === "lose") messageLose.classList.remove("hidden");
}
function handleMove(direction) {
    if (game.getStatus() !== "playing" || !started) return;
    const moved = {
        ArrowUp: ()=>game.moveUp(),
...
document.addEventListener("keydown", (ev)=>{
    if ([
        ...
        "ArrowLeft",
        "ArrowRight"
    ].includes(ev.key)) handleMove(ev.key);
});
let started = false;
   ...
   */ constructor(initialState){
    ...
    move(rows) {
        ...
        for (const row of rows){
            ...
            if (!this.arraysEqual(original, merged)) moved = true;
    ...
    moveDown() {
        ...
        if (moved) {
            this.board = this.transposed(transposedReversed.map((row)=>row.slice().reverse()));
            this.addNewNumber();
            this.updateStatus();
        }
    }
   ...
   */ restart() {
    ...
    addNewNumber() {
        ...
        for(let row1 = 0; row1 < this.size; row1++){
            for(let coll = 0; coll < this.size; coll++)if (this.board[row1][coll] === 0) emptyPosition.push([
                row1,
                coll
            ]);
        }
        if (emptyPosition.length === 0) return;
        const [row, col] = emptyPosition[Math.floor(Math.random() * emptyPosition.length)];
    ...
    updateStatus() {
        ...
        for(let row = 0; row < this.size; row++)for(let cell = 0; cell < this.size; cell++){
            ...
            if (row < this.size - 1 && this.board[row + 1][cell] === value || cell < this.size - 1 && this.board[row][cell + 1] === value) return;
        }
        this.status = "lose";
    }
}
